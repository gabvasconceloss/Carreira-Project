...
export const App = () => {
  ...
  // A PONTE DO RAUL: Guarda o resultado do Quiz para mandar pro Catálogo
  const [eixoDoQuiz, setEixoDoQuiz] = useState<string>("Todas");

  // ESTADOS DO GABRIEL (Animação e Scroll)
  ...
  // FUNÇÃO DE NAVEGAÇÃO SUAVE UNIFICADA
  const navegarPara = (aba: string) => {
    if (aba === "carreiras") {
      setEixoDoQuiz("Todas"); // Reseta o filtro se clicar no menu (Lógica do Raul)
    }
    setAbaAtiva(aba);
  ...
  const renderContent = () => {
    switch (abaAtiva) {
      case "autoconhecimento":
        // O Quiz recebe a função do Gabriel para mudar a tela, e a do Raul para salvar o eixo
        return <Quiz mudarAba={navegarPara} setEixo={setEixoDoQuiz} />;
      case "carreiras":
        // O Catálogo recebe o eixo do Raul!
  ...
  return (
    <div className="app-container">
      {/* HEADER DO GABRIEL */}
      <header
        className={`header ${headerRolado || abaAtiva !== "home" ? "scrolled" : ""}`}
      >
